...
def main(lr, train_path, eval_path, save_path):
    ...
    fig = plt.figure()
    plt.subplot(3, 2, 1)
    plt.scatter(y_val, prediction[:,0])
    plt.xlabel("True content")
    plt.ylabel("Predicted counts")
    plt.subplot(3, 2, 2)
    plt.scatter(y_val, prediction[:,1])
    plt.xlabel("True content")
    plt.ylabel("Predicted counts")
    plt.subplot(3, 2, 3)
    plt.scatter(y_val, prediction[:,2])
    plt.xlabel("True content")
    plt.ylabel("Predicted counts")
    plt.subplot(3, 2, 4)
    plt.scatter(y_val, prediction[:,3])
    plt.xlabel("True content")
    plt.ylabel("Predicted counts")
    plt.subplot(3, 2, 5)
    plt.scatter(y_val, prediction[:,4])
    plt.xlabel("True content")
    plt.ylabel("Predicted counts")
    plt.savefig("poisson.jpg")
    '''
...
class PoissonRegression:
    ...
    def fit(self, x, y):
        ...
        h_x = x @ self.theta
        self.theta = self.theta + self.step_size * (y - (np.exp(h_x))).T @ x
        print("et",self.theta.shape)
        return self.theta
