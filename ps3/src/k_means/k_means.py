...
def init_centroids(num_clusters, image):
    """
    Initialize a `num_clusters` x image_shape[-1] np.array to RGB
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    global new_changes, new_centroids
    height, width, rgb = np.shape(image)
    n_clusters = len(centroids)

    for _ in range(max_iter):
        # Usually expected to converge long before `max_iter` iterations
        if _ == 0 or (_ + 1) % print_every == 0:
            new_centroids = np.zeros(shape=[n_clusters, rgb])
            new_changes = np.zeros(shape=[n_clusters, 1])

        for i in range(height):
            for j in range(width):
                # Initialize `dist` vector to keep track of distance to every centroid
                dist = np.zeros(shape=[n_clusters, 1])

                # Loop over all centroids and store distances in `dist`
                for cluster in range(n_clusters):
                    distance = centroids[cluster, :] - image[i, j, :]
                    dist[cluster] = np.dot(np.transpose(distance), distance)

                # Find closest centroid and update `new_centroids`
                new_changes[dist.argmin()] += 1
                new_centroids[dist.argmin(), :] += image[i, j, :]

        # Update `new_centroids`
        for cluster in range(n_clusters):
            if new_changes[cluster] > 0:
                new_centroids[cluster, :] = new_centroids[cluster, :] / new_changes[cluster]
    # *** END YOUR CODE ***
...
def update_image(image, centroids):
    ...
    for i in range(height):
        for j in range(width):
            ...
            image[i, j, :] = centroids[dist.argmin()]
    # *** END YOUR CODE ***
