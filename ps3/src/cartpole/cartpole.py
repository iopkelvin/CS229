...
def initialize_mdp_data(num_states):
    ...
    # *** START CODE HERE ***
    n_actions = 2 # 0 or 1
    uniform = 1 / num_states
    params = dict(value_function=np.random.randn(num_states) * 0.1,
                  transition_record=np.zeros((num_states, n_actions, num_states)),
                  transition_probabilities=np.ones(([num_states, n_actions, num_states])) * uniform,
                  reward_record=np.zeros(num_states),
                  reward_counts=np.zeros(num_states),
                  reward_values=np.zeros(num_states))
    return params
    # *** END CODE HERE ***
...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    transition_probabilities = mdp_data["transition_probabilities"]
    value_function = mdp_data["value_function"]

    action = np.argmax(np.dot(transition_probabilities[state], value_function))
    '''
    sums = np.sum((transition_probabilities[state]) * value_function, axis=1)
    action = np.argmax(sums)
    return action
    '''
    return action
    # *** END CODE HERE ***
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    # *** START CODE HERE ***
    mdp_data["transition_record"][(state, action, new_state)] += 1
    mdp_data["reward_record"][new_state] = reward #is it reward or += reward?
    mdp_data["reward_counts"] += 1
    # *** END CODE HERE ***
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    transition_record = mdp_data["transition_record"]

    update = np.where(transition_record > 0)[0:2]
    trans_record_update = transition_record[update]
    trans_record_update_sum = np.sum(trans_record_update, axis=1, keepdims=True)
    mdp_data["transition_probabilities"][update] = trans_record_update / trans_record_update_sum
    '''
    transition_record = mdp_data["transition_record"]
    reward_record = mdp_data["reward_record"]
    reward_counts = mdp_data["reward_counts"]

    record = transition_record.sum(axis=2)
    check = record > 0
    update = transition_record[check] / record[check].reshape(-1, 1)
    mdp_data["transition_probabilities"][check] = update

    check = reward_counts > 0
    mdp_data["reward_values"] = reward_record[check] / reward_counts[check]
    '''
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    mdp_data["value_function"] = reward_record + np.max(gamma * np.dot(transition_probabilities, value_function), axis=1)
    diff = np.max(np.abs(old_value_function - mdp_data["value_function"]))
    '''
    sums = np.sum(transition_probabilities * value_function, axis=2)
    max_val = np.max(sums, axis=1)
    new_value_function = (reward_values + gamma * max_val)
